...
func TestRunnerFrameIsIsolated(t *testing.T) {
	...
	if !strings.Contains(run, `setAttribute("sandbox", "allow-scripts")`) {
...
func TestFetchedAssetsArePinned(t *testing.T) {
	manifest := repoFile(t, "assets.sha256")
...
func TestMarkdownSanitizerIsAnAllowlist(t *testing.T) {
	src := []byte(readAsset(t, "static/render.js"))
	js := string(src)
...
func TestVendoredAssetsMatchPins(t *testing.T) {
	pins := bytes.NewReader(repoFile(t, "assets.sha256"))
