...
// The Portuguese translation table.
//
// Data, kept away from the code that reads it: the machinery in i18n.go is
// sixty lines and this is several hundred, and a file where the one is buried
// in the other is a file nobody can skim. English needs no table — the English
// text is the key, so an untranslated string falls back to itself.
//
// The browser is given this same table as JSON from /strings/{lang}, once, and
// keeps it. Pages are built client-side, so there is no per-page rendering step
// on the server for a template to translate in.
...
var translations = map[string]map[string]string{
	"pt-BR": {
		...
		// plots
		"Figure %s":                    "Figura %s",
		"Show the code that drew this": "Mostrar o código que desenhou isto",
		"Hide the code":                "Ocultar o código",
		...
		"A plot block draws itself when the note opens — charts, function graphs, vectors and geometry — as part of the same typesetting that makes your PDFs, so the note and the document show one drawing rather than two that agree.": "Um bloco plot se desenha ao abrir a anotação — gráficos, funções, vetores e geometria — pela mesma tipografia que gera os seus PDFs, então a anotação e o documento mostram um só desenho, e não dois que combinam.",
		"A plot fence is drawn by the typesetter. Start a line with //: to caption it, or //: label=name to give it a name that @name points at from the prose.":                                                                          "Um bloco plot é desenhado pelo tipógrafo. Comece uma linha com //: para legendá-lo, ou //: label=nome para dar um nome que @nome referencia no texto.",
		"Examples copied from the web work as written — the import is pointed at the copy kept here, whichever version it names. Nothing is downloaded, and nothing leaves this device.":                                                  "Exemplos copiados da web funcionam como estão — o import aponta para a cópia guardada aqui, seja qual for a versão que ele nomeia. Nada é baixado, e nada sai deste dispositivo.",
		"For 3D surfaces, or anything numpy, scipy and sympy can compute, write python plot instead. The first of those in a session takes a few seconds while Python starts.":                                                            "Para superfícies em 3D, ou qualquer coisa que numpy, scipy e sympy calculem, escreva python plot. O primeiro deles leva alguns segundos enquanto o Python inicia.",
		"Drawing needs a runtime this server does not have: run `make assets` to add them.":                                                                                                                                               "Desenhar precisa de um runtime que este servidor não tem: rode `make assets` para adicioná-los.",
		"Drawn by the typesetter — instant, and vector in the PDF":                                                                                                                                                                        "Desenhado pelo tipógrafo — instantâneo, e vetorial no PDF",
		"Drawn by matplotlib — numpy, scipy, sympy and 3D": "Desenhado pelo matplotlib — numpy, scipy, sympy e 3D",
		"Directives open a fence as a comment — //: in a plot block, #: in a Python one — so the code still runs if you paste it into a file. A caption may contain math between dollar signs, and figures number themselves in the order they appear.": "As diretivas abrem o bloco como comentário — //: num bloco plot, #: num bloco Python — para que o código ainda rode se você colá-lo num arquivo. Uma legenda pode conter matemática entre cifrões, e as figuras se numeram na ordem em que aparecem.",
		"Reach for python plot when you need 3D, or anything numpy, scipy and sympy can compute. The first of those in a session takes a few seconds while Python starts.":                                                                              "Use python plot quando precisar de 3D, ou de qualquer coisa que numpy, scipy e sympy calculem. O primeiro deles leva alguns segundos enquanto o Python inicia.",
