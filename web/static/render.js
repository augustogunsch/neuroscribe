...
/* The classes highlight.js writes, and the only ones a <span> may carry.
 *
 * Note markup cannot produce a span at all — it is not in NG_ALLOWED_TAGS —
 * so these arrive from exactly one place: the highlighter, whose output is
 * sanitized like anything else before it reaches the page. A scope may be
 * compound ("hljs-title function_"), hence the second, bare form.
 */
const NG_HLJS_CLASS = /^(?:hljs-[a-z_-]+|[a-z_]+_)$/;
			...
			if (node.hasAttribute && node.hasAttribute("class")) {
				const kept = String(node.getAttribute("class")).split(/\s+/)
					.filter((c) => (node.tagName === "CODE" &&
						...
						(node.tagName === "SPAN" && NG_HLJS_CLASS.test(c)));
...
/* ---- syntax highlighting ----
 *
 * It runs here, over the DOM, rather than during the Markdown pass — and that
 * ordering is the whole of the design.
 *
 * A highlighter turns text into markup, which is precisely what this app does
 * not accept from a note. So the note is parsed and sanitized first, and only
 * then is each block's *text* handed to highlight.js. What comes back is a
 * string of HTML built from text that has already been cleaned, and it is
 * cleaned again anyway, against a list that permits one tag: a <span> carrying
 * a highlight class. Nothing else survives, whatever the highlighter does.
 *
 * The colours are not here. They come from /static/chroma.css, generated by
 * internal/app/highlight.go, so that light and dark are one file and the
 * palette is the server's to change.
 */

// Fence languages people write, against what highlight.js calls them. Anything
// it already knows by name needs no entry.
const NG_HLJS_ALIAS = {
	js: "javascript", jsx: "javascript", mjs: "javascript",
	ts: "typescript", tsx: "typescript",
	py: "python", py3: "python", python3: "python",
	sh: "bash", zsh: "bash", shell: "bash", console: "bash",
	yml: "yaml", md: "markdown", rs: "rust", rb: "ruby",
	"c++": "cpp", "c#": "csharp", cs: "csharp", golang: "go",
	html: "xml", svg: "xml", vue: "xml", htm: "xml",
	tex: "latex", typ: "typst", plot: "typst",
};

function ngHljsLanguage(lang) {
	const name = String(lang || "").toLowerCase();
	if (!name) return "";
	const mapped = NG_HLJS_ALIAS[name] || name;
	return typeof hljs !== "undefined" && hljs.getLanguage(mapped) ? mapped : "";
}

function ngHighlight(root) {
	if (typeof hljs === "undefined") return; // unhighlighted code is readable
	(root || document).querySelectorAll("pre > code").forEach(function (code) {
		if (code.dataset.highlighted) return;
		const match = /language-([A-Za-z0-9+#._-]+)/.exec(code.className || "");
		const language = ngHljsLanguage(match && match[1]);
		// Only a language we were told and recognise. Guessing is worse than
		// leaving it plain: a wrong guess colours words that mean nothing.
		if (!language) return;
		code.dataset.highlighted = "1";
		let markup;
		try {
			markup = hljs.highlight(code.textContent, {
				language: language,
				ignoreIllegals: true, // a snippet mid-edit is not an error
			}).value;
		} catch (err) {
			return; // the block keeps its text
		}
		const clean = ngSanitizer().sanitize(markup, {
			ALLOWED_TAGS: ["span"],
			ALLOWED_ATTR: ["class"],
			ALLOW_DATA_ATTR: false,
			ALLOW_ARIA_ATTR: false,
